...
while rodando:
    ...
    if spawn_timer > 40:
        ...
        spawn_timer = 0
    spawn_timer_LE += 1
    if spawn_timer_LE > 120:
        robo = Robo_LE(random.randint(40, LARGURA - 40), -40)
        todos_sprites.add(robo)
        inimigos.add(robo)
        spawn_timer_LE = 0

    # NOVO BLOCO DE COLISÃO
    colisoes_tiro_inimigo = pygame.sprite.groupcollide(inimigos, tiros, True, True)
    
    for robo_atingido in colisoes_tiro_inimigo:
        robo_atingido.morreu()
        pontos += 1
    ...
    for explosao in list(explosions):
        explosao.update()
        explosao.draw(TELA)
        if explosao.ended():
